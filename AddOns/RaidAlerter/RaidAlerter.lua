...
function RaidAlerter_SetCameraMaxToMax()
	if RaidAlerter_SET.cameraMax then
		...
		SetCVar("cameraDistanceMaxZoomFactor", 4);		--当前3.05版本极限=2
	end
end
...
function RaidAlerter_Check_Variables()
	...
	if RaidAlerter_SET.MAXRAM == nil then RaidAlerter_SET.MAXRAM = 10240; end
	...
	if RAL_PvPPlayerCount<RaidNum/2 then
		...
		if check=="tip" then
			...
			if RAL_checkHEALERlistPlayer ~= "" then
				RAL.SendMSG("治疗"..RAL_checkHEALERlistPlayerNum.."名:　"..RAL_checkHEALERlistPlayer,AlertChan,1);
			end
		end
		-- new add end
		if RAL_PvPPlayerCount>0 then
