...
--dictionary
local IGNORE_UNAIVALABLE_FOLLOWERS= L["Ignore unavailable"]
local IGNORE_UNAIVALABLE_FOLLOWERS_DETAIL= L["Always ignore followers in mission, work and idle."]
...
function addon:OnInitialized()
	...
	self:AddLabel(L["Buildings Panel"])
...
function addon:AddIndicatorToButton(button,mission,missionID,bigscreen)
	...
	if (GMFMissions.showInProgress) then
		...
		panel.Percent:SetJustifyH("RIGHT")
		panel.Age:SetFormattedText(L["Expires in \n%s"],mission.offerTimeRemaining)
