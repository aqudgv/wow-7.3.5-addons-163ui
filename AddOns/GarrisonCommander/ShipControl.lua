	...
	function module:CalculateMissions(this,elapsed)
		...
		if (timeElapsed >=0.05) then
			...
			if (currentMission > #aMissions) then
				...
				GMC.list.widget:SetFormattedTitle(L["Processing mission %d of %d (%s)"],currentMission,#aMissions,G.GetMissionName(missionID))
				local class=self:GetMissionData(missionID,"class")
...
function module:OnClick_Start(this,button)
	...
	if (shipyard:GetTotFollowers(AVAILABLE) == 0) then
		GMC.list.widget:SetTitle(L["All followers are busy"])
	...
	if (#aMissions>0) then
		...
		GMC.list.widget:SetTitle(L["No mission matches your criteria"])
		GMC.list.widget:SetTitleColor(C.Red())
	end
...
local function drawItemButtons(frame)
	...
	if not GMC.rewardinfo then
		...
		info:SetText(L["Click to enable/disable a reward. Drag to reorder"])
...
function module:BuildMissionList()
	...
	GMC.startButton:SetText(L['Calculate'])
	...
	GMC.startButton:SetScript('OnEnter', function() GameTooltip:SetOwner(GMC.startButton, 'ANCHOR_TOPRIGHT') GameTooltip:AddLine(L['Assign your followers to missions.']) GameTooltip:Show() end)
	...
	GMC.runButton:SetText(L['Send all mission at once'])
