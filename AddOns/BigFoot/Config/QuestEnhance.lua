...
function QuestEnhanceConfigFunc()
	if (GetLocale() == "zhCN") then
		...
		ENABLE_WorldQuestTracker_TOOLTIP = "在破碎群岛地图上汇总所有世界任务";
	...
	elseif (GetLocale() == "zhTW") then
		...
		ENABLE_WorldQuestTracker_TOOLTIP = "";
	...
	if IsConfigurableAddOn("Wholly") then
		...
		local check = CreateFrame("CheckButton", "WhollyLoaderCheckButton", WorldMapDetailFrame, "InterfaceOptionsBaseCheckButtonTemplate");
		check:SetWidth(26);
		check:SetHeight(26);
		check:SetPoint("TOPRIGHT", WorldMapDetailFrame, "TOPRIGHT", -168, -44);
