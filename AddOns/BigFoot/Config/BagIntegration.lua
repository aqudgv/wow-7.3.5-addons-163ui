...
function BagIntegrationConfigFunc()
	if (GetLocale() == "zhCN") then
		...
		Enable_Bagnon = "启用整合背包(Bagnon)"

		COMBUCTOR_DISABLE_DELAY_TEXT ="|cff00c0c0<背包整合>|r 你已经关闭背包整合(Combuctor)模块，该设置将在下次插件载入时生效。";
		Bagnon_DISABLE_DELAY_TEXT ="|cff00c0c0<整合背包>|r 你已经关闭整合背包(Bagnon)模块，该设置将在下次插件载入时生效。";
	elseif (GetLocale()=="zhTW") then
		...
		Bagnon_DISABLE_DELAY_TEXT = "|cff00c0c0<整合背包>|r 你已經關閉整合(Bagnon)模組，該設置將在下次外掛程式載入時生效。";
	else
	...
	local function load_Bagnon()
		BigFoot_LoadAddOn("BagBrother");
		BigFoot_LoadAddOn("Bagnon");
	end

	if IsConfigurableAddOn("Combuctor") or IsConfigurableAddOn("Bagnon") then
	...
	if IsConfigurableAddOn("Combuctor") then
		ModManagement_RegisterCheckBox(
			...
			function (arg)
				if (arg == 1) then
					if (IsConfigurableAddOn("Bagnon")) then
						if (BigFoot_GetModVariable("BagIntegration", "Enable_Bagnon") ~= 0) then
							BigFoot_SetModVariable("BagIntegration", "Enable_Bagnon", 0);
							ModManagement_Refresh();
							if BigFoot_IsAddOnLoaded("Bagnon") then
								BigFoot_RequestReloadUI(function()
									BigFoot_SetModVariable("BagIntegration", "Enable_Bagnon", 1);
									BigFoot_SetModVariable("BagIntegration", "Enable_Combuctor", 0);
									ModManagement_Refresh();
								end);
							end
						else
							if (not BigFoot_IsAddOnLoaded("Bagnon")) then
	...
	if IsConfigurableAddOn("Bagnon") then
		ModManagement_RegisterCheckBox(
			...
			function (arg)
				if (arg == 1) then
					if (IsConfigurableAddOn("Combuctor")) then
						if (BigFoot_GetModVariable("BagIntegration", "Enable_Combuctor") ~= 0) then
							BigFoot_SetModVariable("BagIntegration", "Enable_Combuctor", 0);
							ModManagement_Refresh();
							if BigFoot_IsAddOnLoaded("Combuctor") then
								BigFoot_RequestReloadUI(function()
									BigFoot_SetModVariable("BagIntegration", "Enable_Combuctor", 1);
									BigFoot_SetModVariable("BagIntegration", "Enable_Bagnon", 0);
									ModManagement_Refresh();
								end);
							end
					...
					if (BigFoot_IsAddOnLoadedFromBigFoot("Bagnon")) then
						BigFoot_RequestReloadUI(function() BigFoot_Print(Bagnon_DISABLE_DELAY_TEXT); end);
					end
				end
			end
		);
