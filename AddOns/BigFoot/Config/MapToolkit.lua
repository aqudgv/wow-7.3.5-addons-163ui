...
function MapToolkitConfigFunc()
	...
	if IsConfigurableAddOn("MapPlus") or IsConfigurableAddOn("BigFootMark") or IsConfigurableAddOn("Archy")  or IsConfigurableAddOn("SexyMap") or IsConfigurableAddOn("RangeDisplay") or IsConfigurableAddOn("Mapster") then
	...
	if IsConfigurableAddOn("Mapster") and not IsAddOnLoaded('WorldFlightMap') then
		ModManagement_RegisterCheckBox(
			"MapToolkit",
			...
			function (__arg)
				if (__arg==1)then
					if (not BigFoot_IsAddOnLoaded("Mapster")) then
						BigFoot_LoadAddOn("Mapster");
					end
					BFWorldFlightMap_Toggle(true);
				else
					if BigFoot_IsAddOnLoaded("Mapster") then
