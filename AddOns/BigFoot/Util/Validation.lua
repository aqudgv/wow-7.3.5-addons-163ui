...
function e:PLAYER_LOGIN()
	if not isValid(BF_VERSION_CHECKSUM.."|"..BIGFOOT_VERSION) then
		...
		RegisterAddonMessagePrefix("BF_VER_CHK");
...
function e:GROUP_ROSTER_UPDATE()
	if not hasSend and IsInRaid() then
		...
		-- end
		BigFoot_DelayCall(function()
			SendAddonMessage("BF_VER_CHK",BF_VERSION_CHECKSUM.."|"..BIGFOOT_VERSION,(IsInGroup(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT") or "RAID")
		end,3);
		hasSend = true;
	else
		...
		if GetNumSubgroupMembers() >= 1 then
			...
			if not hasSend then
				BigFoot_DelayCall(function()
					SendAddonMessage("BF_VER_CHK",BF_VERSION_CHECKSUM.."|"..BIGFOOT_VERSION,(IsInGroup(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT") or "PARTY")
				end,3);
				hasSend = true;
			end
			return;
