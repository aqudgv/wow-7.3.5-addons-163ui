...
function CtlRegularTip(self, cfg)
    ...
    if cfg.reload and not cfg._reloadTipAdded then
        if not cfg.tipLines then cfg.tipLines = { L["Hint"] } else tinsert(cfg.tipLines, " ") end
        tinsert(cfg.tipLines, "|cffff0000"..L["Changing this option requires an UI reload"].."|r");
