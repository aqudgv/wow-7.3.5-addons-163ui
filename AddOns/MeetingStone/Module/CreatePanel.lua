...
function CreatePanel:OnInitialize()
    ...
    local ActivityType = GUI:GetClass('Dropdown'):New(ActivityOptions) do
        ...
        ActivityType:SetCallback('OnSelectChanged', function(_, item)
            ...
            self.ActivityMode:SetValue(DEFAULT_MODE_LIST[item.value] or DEFAULT_MODE_LIST[item.categoryId] or 1)
            self.ActivityLoot:SetValue(DEFAULT_LOOT_LIST[item.value] or DEFAULT_LOOT_LIST[item.categoryId] or 1)
            self:InitProfile()
            self:UpdateControlState()
        end)
    end
