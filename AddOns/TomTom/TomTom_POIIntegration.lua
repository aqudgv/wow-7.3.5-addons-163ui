...
local wp_title, wp_x, wp_y
if self.worldQuest then
    local questID = self.questID
    local taskInfo = C_TaskQuest.GetQuestsForPlayerByMapID(m)
    if ( #taskInfo > 0 ) then
        for i, info  in ipairs(taskInfo) do
            if ( info.questId == questID ) then
                wp_title, wp_x, wp_y = C_TaskQuest.GetQuestInfoByQuestID(questID), info.x, info.y
            end
        end
    end
    if wp_title == nil then return end
    ...
    wp_title, wp_x, wp_y = title, x, y
end
local title, x, y = wp_title, wp_x, wp_y
